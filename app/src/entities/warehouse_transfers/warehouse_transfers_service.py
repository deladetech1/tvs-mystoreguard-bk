...
class WarehouseTransfersService:
    ...
    def create_warehouse_transfer(
        ...
        """Create a new warehouse transfer request with one or more items"""
        logger.info(
        ...
        try:
            with DatabaseManager.transaction() as cursor:
                ...
                # Validate every item: product exists, present in source warehouse, sufficient FIFO qty
                ...
                for item in data.items:
                    ...
                    if not source_warehouse_product:
                        return Respons(
                            success=False,
                            detail=f"Product {product.get('name', item.product_id)} not found in source warehouse location",
                ...
                try:
                    ActivityLogService.log_activity(
                        ...
                        description=f"Warehouse transfer {transfer_number} created successfully with {len(validated_items)} item(s)",
    ...
    def _execute_warehouse_transfer(
        ...
        try:
            ...
            # Move each item
            for item in items:
                moved = WarehouseTransfersService._move_product_batches(
                    tenant_id=tenant_id,
                    org_id=org_id,
                    bus_id=bus_id,
                    transfer=transfer,
