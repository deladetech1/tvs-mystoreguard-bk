...
class CreateSaleWriteBase(BaseModel):
    ...
    verified_total_amount: Optional[float] = Field(None, ge=0, description="total_amount from verify_price (promo discount already applied per-item)")
    verified_promo_discount_amount: Optional[float] = Field(None, ge=0, description="promo_discount_amount from verify_price (for record-keeping only)")
    verified_final_total_amount: Optional[float] = Field(None, ge=0, description="final_total_amount from verify_price (amount customer pays — same as verified_total_amount)")
    verified_subtotal_before_discount: Optional[float] = Field(None, ge=0, description="subtotal_before_discount from verify_price (total before promo, with tax). Used for promo usage tracking.")
