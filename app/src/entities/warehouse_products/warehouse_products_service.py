...
from src.entities.warehouse_products.warehouse_products_read_dto import (
    CreateWarehouseProductServiceReadDto,
    UpdateWarehouseProductServiceReadDto,
    DeleteWarehouseProductServiceReadDto,
    PermanentDeleteWarehouseProductServiceReadDto,
    ReverseBatchWarehouseProductServiceReadDto,
    GetWarehouseProductServiceReadDto,
...
from src.entities.warehouse_products.warehouse_products_write_dto import (
    CreateWarehouseProductServiceWriteDto,
    UpdateWarehouseProductServiceWriteDto,
    DeleteWarehouseProductServiceWriteDto,
    PermanentDeleteWarehouseProductServiceWriteDto,
    ReverseBatchWarehouseProductServiceWriteDto,
    AddStockWarehouseProductServiceWriteDto,
)
...
class WarehouseProductsService:
    """Service class for warehouse products operations"""

    @staticmethod
    def _get_warehouse_product_batches(
        tenant_id: str,
        org_id: str,
        bus_id: str,
        loc_id: str,
    ...
    @staticmethod
    def _get_warehouse_product_documents(
        cursor,
        tenant_id: str,
        org_id: str,
        bus_id: str,
        ...
        """Get documents for a product (used by warehouse products) with presigned URLs"""
        cursor.execute(
    ...
    @staticmethod
    def _get_warehouse_product_movements(
        tenant_id: str,
        org_id: str,
        bus_id: str,
        loc_id: str,
        ...
        """Get product movements for a warehouse location (DEPRECATED - use _get_batch_movements instead)"""
        cursor.execute(
            ...
            AND pm.product_id = %s AND pm.location_id = %s AND pm.location_type = 'WAREHOUSE'
    ...
    def _get_batch_movements(
        ...
        """Get movements for a specific batch at a warehouse location (only WAREHOUSE location_type)"""
        cursor.execute(
            f"""SELECT pm.*, pb.batch_number,
                   p.name as product_name,
                   l.loc_name as location_name,
            ...
            LEFT JOIN {db_settings.CORE_PLATFORM_USERS_TABLE} deleter 
                ON pm.deleted_by = deleter.id AND pm.tenant_id = deleter.tenant_id
            WHERE pm.tenant_id = %s AND pm.org_id = %s AND pm.bus_id = %s 
            AND pm.batch_id = %s AND pm.location_type = 'WAREHOUSE' AND pm.location_id = %s
            ORDER BY pm.cdatetime DESC""",
    ...
    def create_warehouse_product(
        data: CreateWarehouseProductServiceWriteDto,
        tenant_id: str,
        org_id: str,
        bus_id: str,
        loc_id: str,
        created_by: str
    ) -> Respons[CreateWarehouseProductServiceReadDto]:
        """Create a new warehouse product with FIFO batch allocation"""
        logger.info(
            f"Processing warehouse product creation",
            extra={
                "extra_fields": {
                    "tenant_id": tenant_id,
                    "org_id": org_id,
        ...
        try:
            with DatabaseManager.transaction() as cursor:
                ...
                for batch in batches_to_use:
                        ...
                        qty_to_allocate = min(remaining_qty_to_allocate, current_batch_qty_remaining)
                        ...
                        cursor.execute(
                            ...
                            WHERE tenant_id = %s AND org_id = %s AND bus_id = %s 
                            AND loc_id = %s AND purchase_batche_id = %s AND location_type = 'WAREHOUSE'""",
                        ...
                        if existing_batch_location:
                            ...
                            new_qty = existing_batch_location['qty'] + qty_to_allocate
                            cursor.execute(
                            ...
                            cursor.execute(
                                ...
                                (
                                    ...
                                    batch_id, 'WAREHOUSE', qty_to_allocate,
                        ...
                        new_batch_qty_remaining = current_batch_qty_remaining - qty_to_allocate
                        cursor.execute(
                        ...
                        batch_allocations.append((batch_id, qty_to_allocate))
                        remaining_qty_to_allocate -= qty_to_allocate

                # Insert or update warehouse product
                if is_update:
                    # Update existing warehouse product - only update current_qty, don't update reorder_level, reorder_quantity, comment, or is_active
                    cursor.execute(
                    ...
                    cursor.execute(
                        f"""SELECT * FROM {db_settings.MSG_WAREHOUSE_PRODUCTS_TABLE}
                        WHERE tenant_id = %s AND org_id = %s AND bus_id = %s 
                        AND loc_id = %s AND product_id = %s""",
                        (tenant_id, org_id, bus_id, loc_id, data.product_id),
                    )
                    warehouse_product_result = cursor.fetchone()
                else:
                    # Insert new warehouse product
                    cursor.execute(
                        f"""INSERT INTO {db_settings.MSG_WAREHOUSE_PRODUCTS_TABLE}
                        (tenant_id, org_id, bus_id, loc_id, product_id, current_qty, comment,
                ...
                if not warehouse_product_result:
                    raise ValueError("Failed to create/update warehouse product")
                ...
                for batch_id, qty_allocated in batch_allocations:
                    ...
                    cursor.execute(
                        ...
                        (
                            ...
                            'OUT', qty_allocated, 'Warehouse product allocation', None,
                            cdate, ctime, cdatetime, created_by
                        ),
                    )
                    
                    # Create IN movement (product going into the warehouse)
                    ...
                    cursor.execute(
                        ...
                        (
                            ...
                            batch_id, 'WAREHOUSE', loc_id,
                            'IN', qty_allocated, 'Warehouse product allocation', None,
                            cdate, ctime, cdatetime, created_by
                        ),
                    )

                # Get warehouse product with user fullnames
                cursor.execute(
                    f"""SELECT sp.*,
                           ...
                           p.is_active,
                           l.loc_name as location_name
                    FROM {db_settings.MSG_WAREHOUSE_PRODUCTS_TABLE} sp
                ...
                if sp_with_users:
                    ...
                    sp_dict = dict(warehouse_product_result)
                    sp_dict['created_by'] = None
                    sp_dict['updated_by'] = None
                    sp_dict['deleted_by'] = None
                    sp_dict['product_name'] = None
                    sp_dict['location_name'] = None
                    # Product fields - need to fetch from product table if warehouse_product_result exists
                    if warehouse_product_result and warehouse_product_result.get('product_id'):
                        cursor.execute(
                            f"""SELECT name, description, sku, bar_code, is_active
                            FROM {db_settings.MSG_PRODUCTS_TABLE}
                            WHERE id = %s AND tenant_id = %s AND org_id = %s AND bus_id = %s""",
                            (warehouse_product_result['product_id'], tenant_id, org_id, bus_id),
                        )
                ...
                try:
                    ...
                    sp_dict['price_after_tax'] = None  # No tax for warehouse products
                ...
                try:
                    ...
                    ActivityLogService.log_activity(
                        ...
                        comment=f"Warehouse product {sp_dict['id']} {action}d successfully",
                ...
                logger.info(
                    f"Warehouse product {action_msg} successfully: {sp_dict['id']}",
                    extra={
                        "extra_fields": {
                            "warehouse_product_id": sp_dict['id'],
                            "product_id": data.product_id,
                            "loc_id": loc_id,
                            "action": action_msg,
                        }
                ...
                return Respons(
                    success=True,
                    detail=f"Warehouse product {action_msg} successfully",
                    data=[sp_read],
                )

        except ValueError as e:
            logger.error(f"Validation error creating warehouse product: {str(e)}")
            return Respons(
                success=False,
                detail=str(e),
            ...
            logger.error(f"Error creating warehouse product: {str(e)}", exc_info=True)
            return Respons(
                success=False,
    ...
    def add_stock_warehouse_product(
        data: AddStockWarehouseProductServiceWriteDto,
        tenant_id: str,
        org_id: str,
        bus_id: str,
        loc_id: str,
        created_by: str
    ) -> Respons[AddStockWarehouseProductServiceReadDto]:
        """Add stock to an existing warehouse product with FIFO batch allocation"""
        logger.info(
            f"Processing add stock to warehouse product",
            extra={
                "extra_fields": {
                    "tenant_id": tenant_id,
                    "org_id": org_id,
        ...
        try:
            with DatabaseManager.transaction() as cursor:
                ...
                # Check if warehouse product exists - MUST exist for add stock
                cursor.execute(
                    f"""SELECT id, current_qty FROM {db_settings.MSG_WAREHOUSE_PRODUCTS_TABLE}
                    WHERE tenant_id = %s AND org_id = %s AND bus_id = %s 
                    AND loc_id = %s AND product_id = %s AND delete_status = 'NOT_DELETED'""",
                ...
                if not existing:
                    return Respons(
                        success=False,
                        detail="Warehouse product does not exist. Use create endpoint to add a new product.",
                ...
                for batch in batches_to_use:
                    ...
                    if existing_batch_location:
                        ...
                        cursor.execute(
                            ...
                            (
                                batch_location_id, tenant_id, org_id, bus_id, loc_id,
                                batch_id, 'WAREHOUSE', qty_to_allocate_batch,
                                batch_cdate, batch_ctime, batch_cdatetime_for_insert
                            ),
                        )
                ...
                # Update warehouse product - only update current_qty
                cursor.execute(
                    f"""UPDATE {db_settings.MSG_WAREHOUSE_PRODUCTS_TABLE}
                    SET current_qty = %s, updated_by = %s
                    WHERE tenant_id = %s AND org_id = %s AND bus_id = %s 
                ...
                if not warehouse_product_result:
                    raise ValueError("Failed to update warehouse product")
                ...
                for batch_id, qty_allocated in batch_allocations:
                    ...
                    cursor.execute(
                        ...
                        (
                            out_movement_id, tenant_id, org_id, bus_id, data.product_id,
                            batch_id, 'SYSTEM', None,
                            'OUT', qty_allocated, 'Add stock to warehouse product', None,
                            cdate, ctime, cdatetime, created_by
                        ),
                    )
                    ...
                    cursor.execute(
                        ...
                        (
                            in_movement_id, tenant_id, org_id, bus_id, data.product_id,
                            batch_id, 'WAREHOUSE', loc_id,
                            'IN', qty_allocated, 'Add stock to warehouse product', None,
                            cdate, ctime, cdatetime, created_by
                        ),
                    )
                ...
                sp_read = AddStockWarehouseProductServiceReadDto(**sp_dict)
                ...
                try:
                    ...
                    ActivityLogService.log_activity(
                        tenant_id=tenant_id,
                        resource_type="rt-warehouse-products",
                        resource_id=sp_dict['id'],
                        action="add_stock",
                        old_data=old_data,
                        new_data=sp_dict,
                        description=f"Added {qty_to_allocate} units to warehouse product {sp_dict['id']}",
                        performed_by=created_by,
                        org_id=org_id,
                        bus_id=bus_id,
                        cursor=cursor
                ...
                logger.info(
                    f"Stock added to warehouse product successfully: {sp_dict['id']}",
                    extra={
                        "extra_fields": {
                            "warehouse_product_id": sp_dict['id'],
                            "product_id": data.product_id,
                ...
                return Respons(
                    success=True,
                    detail=f"Successfully added {qty_to_allocate} units to warehouse product",
                    data=[sp_read],
                )

        except ValueError as e:
            logger.error(f"Validation error adding stock to warehouse product: {str(e)}")
            return Respons(
                success=False,
                detail=str(e),
                error="VALIDATION_ERROR",
            ...
            logger.error(f"Error adding stock to warehouse product: {str(e)}", exc_info=True)
            return Respons(
                success=False,
                detail=f"Failed to add stock to warehouse product: {str(e)}",
                error="INTERNAL_ERROR",
            )

    @staticmethod
    def update_warehouse_product(
        data: UpdateWarehouseProductServiceWriteDto,
        loc_id: str,
        product_id: str,
        tenant_id: str,
        org_id: str,
        bus_id: str,
        updated_by: str
    ) -> Respons[UpdateWarehouseProductServiceReadDto]:
        """Update a warehouse product"""
        logger.info(
            f"Processing warehouse product update: loc_id={loc_id}, product_id={product_id}",
            extra={
                "extra_fields": {
                    "loc_id": loc_id,
        ...
        try:
            with DatabaseManager.transaction() as cursor:
                # Fetch existing warehouse product
                cursor.execute(
                    f"""SELECT * FROM {db_settings.MSG_WAREHOUSE_PRODUCTS_TABLE}
                    WHERE tenant_id = %s AND org_id = %s AND bus_id = %s 
                    AND loc_id = %s AND product_id = %s AND delete_status = 'NOT_DELETED'""",
                ...
                if not existing_sp:
                    return Respons(
                        success=False,
                        detail="Warehouse product not found",
                        error="NOT_FOUND",
                    )
                ...
                # Get updated warehouse product with user fullnames
                cursor.execute(
                    f"""SELECT sp.*,
                           creator.fullname as created_by,
                           updater.fullname as updated_by,
                ...
                try:
                    ...
                    ActivityLogService.log_activity(
                        ...
                        old_data=old_data,
                        new_data=complete_new_data,
                        comment=f"Warehouse product {sp_dict['id']} updated successfully",
                ...
                logger.info(
                    f"Warehouse product updated successfully: {sp_dict['id']}",
                    extra={
                        "extra_fields": {
                            "warehouse_product_id": sp_dict['id'],
                        }
                ...
                return Respons(
                    success=True,
                    detail="Warehouse product updated successfully",
                    data=[sp_read],
                )

        except Exception as e:
            logger.error(f"Error updating warehouse product: {str(e)}", exc_info=True)
            return Respons(
                success=False,
                detail=f"Failed to update warehouse product: {str(e)}",
                error="INTERNAL_ERROR",
            )
    ...
    def get_warehouse_product(
        ...
        org_id: str,
        bus_id: str,
    ) -> Respons[GetWarehouseProductServiceReadDto]:
        """Get a single warehouse product by location and product ID"""
        logger.info(
            f"Processing get warehouse product request: loc_id={loc_id}, product_id={product_id}",
            extra={
                "extra_fields": {
                    "loc_id": loc_id,
                    "product_id": product_id,
        ...
        try:
            with DatabaseManager.transaction() as cursor:
                ...
                sp_dict['documents'] = WarehouseProductsService._get_warehouse_product_documents(
                    cursor=cursor,
                    tenant_id=tenant_id,
                    org_id=org_id,
                    bus_id=bus_id,
                ...
                # Get batches for this warehouse product
                batches_data = WarehouseProductsService._get_warehouse_product_batches(
                    tenant_id=tenant_id,
                    org_id=org_id,
                    bus_id=bus_id,
                    loc_id=loc_id,
                ...
                batches_list = []
                # Note: remaining_qty_total will be set from wp_dict['current_qty'] which is the total quantity at this location
                for batch_loc in batches_data:
                    batch_dict = dict(batch_loc)
                    batch_dict['batch_number'] = batch_dict.get('batch_number') or None
                    
                    # Get the quantity allocated to THIS warehouse location (from batch_locations table)
                    ...
                    movements_data = []
                    if batch_id:
                        movements_data = WarehouseProductsService._get_batch_movements(
                            tenant_id=tenant_id,
                            org_id=org_id,
                            bus_id=bus_id,
                            batch_id=batch_id,
                    ...
                    if batch_dict.get('purchase_batche_id'):
                        ...
                        batch_details_dict = {
                            ...
                            'qty_remaining': None,  # Not used for warehouse products
                            ...
                            'qty_at_location': int(qty_at_location) if qty_at_location else None,  # Quantity at THIS warehouse location
                ...
                # Use current_qty from warehouse_products table (total quantity at this location)
                ...
                return Respons(
                    success=True,
                    detail="Warehouse product retrieved successfully",
                    data=[sp_read],
                )

        except Exception as e:
            logger.error(f"Error getting warehouse product: {str(e)}", exc_info=True)
            return Respons(
                success=False,
                detail=f"Failed to get warehouse product: {str(e)}",
                error="INTERNAL_ERROR",
            )
    ...
    def get_warehouse_products(
        ...
        """Get list of warehouse products with pagination"""
        logger.info(
            f"Processing get warehouse products request",
            extra={
                "extra_fields": {
                    "tenant_id": tenant_id,
                    "loc_id": loc_id,
        ...
        try:
            with DatabaseManager.transaction() as cursor:
                ...
                if search:
                    where_conditions.append(
                        "(p.name ILIKE %s OR sp.comment ILIKE %s OR l.name ILIKE %s OR p.bar_code ILIKE %s)"
                    )
                ...
                # Get total count
                cursor.execute(
                    f"""SELECT COUNT(*) as total FROM {db_settings.MSG_WAREHOUSE_PRODUCTS_TABLE} sp
                    LEFT JOIN {db_settings.MSG_PRODUCTS_TABLE} p ON sp.product_id = p.id 
                        AND sp.tenant_id = p.tenant_id 
                        AND sp.org_id = p.org_id 
                        AND sp.bus_id = p.bus_id
                ...
                # Get warehouse products with user fullnames
                cursor.execute(
                    f"""SELECT sp.*,
                           creator.fullname as created_by,
                           updater.fullname as updated_by,
            ...
            logger.error(f"Error getting warehouse products: {str(e)}", exc_info=True)
            return Respons(
                success=False,
                detail=f"Failed to get warehouse products: {str(e)}",
                error="INTERNAL_ERROR",
            )

    @staticmethod
    def delete_warehouse_product(
        data: DeleteWarehouseProductServiceWriteDto,
        tenant_id: str,
        org_id: str,
        bus_id: str,
        deleted_by: str
    ) -> Respons[DeleteWarehouseProductServiceReadDto]:
        """Delete a warehouse product"""
        logger.info(
            f"Processing warehouse product deletion: loc_id={data.loc_id}, product_id={data.product_id}",
            extra={
                "extra_fields": {
                    "loc_id": data.loc_id,
                    "product_id": data.product_id,
        ...
        try:
            with DatabaseManager.transaction() as cursor:
                ...
                cursor.execute(
                    ...
                    WHERE tenant_id = %s AND org_id = %s AND bus_id = %s 
                    AND loc_id = %s AND location_type = 'WAREHOUSE'
                ...
                try:
                    ...
                    ActivityLogService.log_activity(
                        ...
                        old_data=old_data,
                        new_data=None,
                        description=f"Warehouse product deleted successfully",
                ...
                logger.info(
                    f"Warehouse product deleted successfully: loc_id={data.loc_id}, product_id={data.product_id}. "
                ...
                return Respons(
                    success=True,
                    detail="Warehouse product deleted successfully",
                    data=[DeleteWarehouseProductServiceReadDto(
                        loc_id=data.loc_id,
                        product_id=data.product_id,
            ...
            logger.error(f"Error deleting warehouse product: {str(e)}", exc_info=True)
            return Respons(
                success=False,
                detail=f"Failed to delete warehouse product: {str(e)}",
                error="INTERNAL_ERROR",
            )

    @staticmethod
    def permanent_delete_warehouse_product(
        data: PermanentDeleteWarehouseProductServiceWriteDto,
        tenant_id: str,
        org_id: str,
        bus_id: str,
        loc_id: str,
        deleted_by: str
    ) -> Respons[PermanentDeleteWarehouseProductServiceReadDto]:
        """Permanently delete a warehouse product from the database"""
        logger.info(
            f"Processing permanent delete warehouse product: loc_id={loc_id}, product_id={data.product_id}",
            extra={
                "extra_fields": {
                    "loc_id": loc_id,
                    "product_id": data.product_id,
        ...
        try:
            with DatabaseManager.transaction() as cursor:
                ...
                # Permanently delete the warehouse product
                cursor.execute(
                    f"""DELETE FROM {db_settings.MSG_WAREHOUSE_PRODUCTS_TABLE}
                    WHERE tenant_id = %s AND org_id = %s AND bus_id = %s 
                    AND loc_id = %s AND product_id = %s""",
                    (tenant_id, org_id, bus_id, loc_id, data.product_id),
                )
                ...
                if cursor.rowcount == 0:
                    raise ValueError("Failed to permanently delete warehouse product")
                ...
                try:
                    ActivityLogService.log_activity(
                        ...
                        old_data=old_data,
                        new_data=None,
                        description=f"Warehouse product permanently deleted from location {loc_id}",
                        performed_by=deleted_by,
                        org_id=org_id,
                        bus_id=bus_id,
                        cursor=cursor
                ...
                logger.info(
                    f"Warehouse product permanently deleted successfully: loc_id={loc_id}, product_id={data.product_id}. "
                    f"Deleted {deleted_batch_locations_count} batch location(s) and {deleted_movements_count} movement(s) (IN and OUT).",
                    extra={
                        "extra_fields": {
                            "loc_id": loc_id,
                ...
                return Respons(
                    success=True,
                    detail="Warehouse product permanently deleted successfully",
                    data=[PermanentDeleteWarehouseProductServiceReadDto(
                        loc_id=loc_id,
                        product_id=data.product_id,
                        message="Warehouse product permanently deleted successfully"
                    )],
                )

        except Exception as e:
            logger.error(f"Error permanently deleting warehouse product: {str(e)}", exc_info=True)
            return Respons(
                success=False,
                detail=f"Failed to permanently delete warehouse product: {str(e)}",
                error="INTERNAL_ERROR",
            )

    @staticmethod
    def reverse_batch_warehouse_product(
        data: ReverseBatchWarehouseProductServiceWriteDto,
        tenant_id: str,
        org_id: str,
        bus_id: str,
        updated_by: str
    ) -> Respons[ReverseBatchWarehouseProductServiceReadDto]:
        """Reverse a batch allocation from a warehouse product by removing batch_location and restoring batch qty_remaining"""
        logger.info(
            f"Processing reverse batch for warehouse product: loc_id={data.loc_id}, product_id={data.product_id}, batch_number={data.batch_number}",
            extra={
                "extra_fields": {
                    "loc_id": data.loc_id,
                    "product_id": data.product_id,
        ...
        try:
            with DatabaseManager.transaction() as cursor:
                ...
                cursor.execute(
                    ...
                    WHERE tenant_id = %s AND org_id = %s AND bus_id = %s 
                    AND loc_id = %s AND purchase_batche_id = %s AND location_type = 'WAREHOUSE'""",
                    (tenant_id, org_id, bus_id, data.loc_id, batch_id),
                )
                ...
                if not batch_location:
                    return Respons(
                        success=False,
                        detail=f"Batch '{data.batch_number}' is not allocated to this warehouse location",
                        error="NOT_FOUND",
                    )
                ...
                warehouse_product = cursor.fetchone()
                ...
                warehouse_product_dict = dict(warehouse_product)
                current_qty = warehouse_product_dict.get('current_qty', 0)

                # Validate that we're not reversing more than available
                if qty_to_reverse > current_qty:
                    return Respons(
                        success=False,
                        detail=f"Cannot reverse {qty_to_reverse} units. Warehouse product only has {current_qty} units",
                        error="INSUFFICIENT_QUANTITY",
                    )
                ...
                try:
                    ActivityLogService.log_activity(
                        tenant_id=tenant_id,
                        resource_type="rt-warehouse-products",
                        resource_id=warehouse_product_dict['id'],
                        ...
                        new_data={
                            **warehouse_product_dict,
                            "current_qty": warehouse_product_dict.get('current_qty', 0) - qty_to_reverse if warehouse_product_dict.get('current_qty', 0) - qty_to_reverse >= 0 else 0,
                ...
                logger.info(
                    f"Batch reversed successfully for warehouse product: batch_number={data.batch_number}, qty_reversed={qty_to_reverse}",
                    extra={
                        "extra_fields": {
                            "loc_id": data.loc_id,
                            "product_id": data.product_id,
                ...
                return Respons(
                    ...
                    data=[ReverseBatchWarehouseProductServiceReadDto(
                        loc_id=data.loc_id,
                        product_id=data.product_id,
                        batch_number=data.batch_number,
                        qty_reversed=qty_to_reverse,
    ...
    def get_warehouse_product_statistics(
        ...
        bus_id: str,
        loc_id: str,
    ) -> Respons[GetWarehouseProductStatisticsServiceReadDto]:
        ...
        try:
            with DatabaseManager.transaction() as cursor:
                ...
                cursor.execute(
                    ...
                    FROM {db_settings.MSG_WAREHOUSE_PRODUCTS_TABLE}
                    WHERE {where_clause}""",
                    tuple(params),
                )
                ...
                logger.info(
                    f"Warehouse product statistics calculated: total_products={total_products}, "
                ...
                statistics = GetWarehouseProductStatisticsServiceReadDto(
                ...
                return Respons(
                    success=True,
                    detail="Warehouse product statistics retrieved successfully",
                    data=[statistics],
                )

        except Exception as e:
            logger.error(f"Error getting warehouse product statistics: {str(e)}", exc_info=True)
            return Respons(
                success=False,
                detail=f"Failed to get warehouse product statistics: {str(e)}",
                error="INTERNAL_ERROR",
            )
